...
// }
  ...
  */
  ...
  function Movie(name, genre, rating) {
    this.name = name;
    this.genre = genre;
    this.rating = rating; //Rating: 5, 2 | 4+6/2 = 5
  ...
  function User(name, age, email, savedPosts){
    ...
    this.age = age;
    this.email = email,
    this.savedPosts = savedPosts; //Array of objects : [{}, {}, {}, {}]
  }



  User.prototype.addSavedPost = function(id, title, rating){
    const new_obj = {id: id, title: title, rating: rating};
    this.savedPosts.push(new_obj);
  }
  ...
  User.prototype.removeSavedPost = function (number){
    this.savedPosts.forEach(function (item, index){
      if(item.id === number){
  ...
  // Write a prototype method for the User constructor function named changePostRating that will take in two number parameters. The first will be an id (a number) and
   //the second will be the new rating (a number). Use the id to find the matching object in the savedPosts array. Once you find the matching object, update it's rating 
   //score with the new rating parameter.
  
  // Code here
  User.prototype.changePostRating = function (id, score){
    this.savedPosts.forEach(function (item, index){
      if(item.id === id){
        item.rating = score;
      }
